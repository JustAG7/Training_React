...
const company = [
  { name: 'About', href: '/info', icon: InformationCircleIcon },
...
export default function Header() {
  ...
  const [link2, setLink2] = useState(
    window.location.pathname.includes("/info") ? "curr-page" : ""
  );
                        ...
                        <div className="mx-auto grid max-w-7xl gap-y-6 px-4 py-6 sm:grid-cols-2 sm:gap-8 sm:px-6 sm:py-8 lg:grid-cols-4 lg:px-8 lg:py-12 xl:py-16">
                          {solutions.map((item) => (
                            <Link
                              key={item.name}
                              to={item.href}
                              className="-m-3 flex flex-col justify-between rounded-lg p-3 hover:bg-gray-50" className={link3}
                            >
